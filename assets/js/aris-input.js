 ...
 *
 * Adds nothing visible to the page. Values are kept in localStorage and
 * re-applied on every load, so edits survive closing the browser.
 *
 * Editable: Grade and Status cells in both course tables, plus Total Grade
 * Points / GPA / Status in the year summary and in each semester summary.
 */
(function () {
    'use strict';
    ...
    var ORIG = {};   // field id -> value as captured from the clone, before any edit
    ...
    var CSS = [
        ...
        '#ID .ai-panel{background:#fff;width:100%;max-width:920px;max-height:100%;',
    ...
    function inputHTML(f, store) {
        var set = store.hasOwnProperty(f.id);
        var val = set ? store[f.id] : ORIG[f.id];
    ...
    function init() {
        ...
        document.addEventListener('keydown', function (e) {
            ...
            if (buf === WORD) {
                buf = '';
                // Swallow the final keystroke, or it lands in the field we focus.
                e.preventDefault();
                open();
            }
        });
